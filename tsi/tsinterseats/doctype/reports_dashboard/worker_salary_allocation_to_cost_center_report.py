...
@frappe.whitelist()
def download():
    filename = 'Workers Salary - Allocation to Cost Center Report'
...
def title2(args):
    ...
    data = ['Workers Salary - Allocation to cost centers']
    return data
@frappe.whitelist()
def header1(args):
...
def data1(args):
    ...
    sa = frappe.get_all('Salary Slip', {'start_date': args.start_date, 'end_date': args.end_date, 'salary_structure': 'Worker'}, ['department'])
    ...
    for ss, totals in department_totals.items():
        ...
        data.append([ss, salary_structure_count[ss]] + [totals[0]]+ [totals[1]]+ [totals[2]]+ [totals[3]]+ [totals[4]]+ [totals[5]]+ [totals[6]]+[cc]+['']+ [totals[7]]+vv)
    ...
    grand_total_row = ['Total',total_counts] + [
        total_i, total_a, total_ot, total_aot, total_sha, total_gross, total_ab,'','', total_gross_amt, total_vv
    ]
